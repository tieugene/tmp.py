#!/bin/env python3
"""Drag-n-drop rows in table (multirow version).
...
class TableWidgetDragRows(QTableWidget):
    def __init__(self, *args, **kwargs):
        ...
        self.setSelectionMode(QAbstractItemView.ExtendedSelection)  # multiselect; or .SingleSelection?
        self.setSelectionBehavior(QAbstractItemView.SelectRows)
        self.setDragDropMode(QAbstractItemView.InternalMove)

    def dropEvent(self, event: QDropEvent):
        if not event.isAccepted() and event.source() == self:
            drop_row = self.drop_on(event)  # row replace to
            rows = sorted(set(item.row() for item in self.selectedItems()))
            rows_to_move = [[QTableWidgetItem(self.item(row_index, column_index)) for column_index in range(self.columnCount())]
                            for row_index in rows]
            print(rows, drop_row)
            for row_index in reversed(rows):
                self.removeRow(row_index)  # rm moved row (3)
